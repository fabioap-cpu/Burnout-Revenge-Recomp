...
REXCVAR_DEFINE_BOOL(diag_present_rate, false, "Diagnostics",
                    "Log real Present() calls per second, split by guest-output-driven vs. "
                    "UI-thread-driven presents. Useful for catching double-presentation bugs "
...
Presenter::PaintResult D3D12Presenter::PaintAndPresentImpl(bool execute_ui_drawers) {
  ...
  if (REXCVAR_GET(diag_present_rate)) {
    static std::atomic<uint64_t> present_count{0};
    static std::atomic<uint64_t> present_from_guest_thread{0};
    static std::atomic<uint64_t> present_from_ui_thread{0};
    static auto last_report = std::chrono::steady_clock::now();
    present_count.fetch_add(1, std::memory_order_relaxed);
    if (execute_ui_drawers) {
      present_from_ui_thread.fetch_add(1, std::memory_order_relaxed);
    } else {
      present_from_guest_thread.fetch_add(1, std::memory_order_relaxed);
    }
    auto now = std::chrono::steady_clock::now();
    if (now - last_report >= std::chrono::seconds(1)) {
      last_report = now;
      REXLOG_INFO(
          "[DIAG] real_presents_per_sec={} (guest_output_draws={} ui_only_redraws={})",
          present_count.exchange(0, std::memory_order_relaxed),
          present_from_guest_thread.exchange(0, std::memory_order_relaxed),
          present_from_ui_thread.exchange(0, std::memory_order_relaxed));
    }
  }
