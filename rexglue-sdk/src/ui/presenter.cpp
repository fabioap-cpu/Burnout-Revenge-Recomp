...
void Presenter::WaitForUITickFromUIThread() {
  ...
  while (true) {
    ...
    if (dxgi_ui_tick_force_requested_) {
      ...
      // Mark this forced draw as having caught up to the current known
      // vblank count. Without this, a real-vblank-driven wait shortly after
      // would see last_vblank_ > last_draw_ (still stale from before this
      // forced draw) and fire an extra, redundant real Present() for content
      // that this forced draw already displayed - doubling the real present
      // rate on high-refresh-rate displays (e.g. 180 Hz real vblank + 60 Hz
      // guest-forced draws = 240 measured presents/sec instead of 180).
