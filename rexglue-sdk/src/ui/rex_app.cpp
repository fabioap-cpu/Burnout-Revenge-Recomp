  ...
  if (imgui_drawer_) {
    ...
    if (input_sys) {
      input_sys->SetActiveCallback([this]() {
        // While any overlay is open, release the mouse unconditionally --
        // gating this on WantCaptureMouse deadlocks: the cursor is hidden
        // and re-centered every frame while captured, so ImGui never sees
        // it move onto a widget to report WantCaptureMouse=true in the
        // first place, and the overlay becomes unclickable.
        return !debug_overlay_ && !console_overlay_ && !settings_overlay_ &&
               !achievements_overlay_;
      });
    }
  }
...
void ReXApp::SetupOverlays(rex::ui::Presenter* presenter, rex::ui::ImmediateDrawer* drawer) {
  ...
  rex::ui::RegisterBind("bind_checkpoint_probe", "F5", "Probe guest checkpoint boundary", [] {
    ...
    } else if (result.thread_found) {
      REXLOG_WARN(
          "Checkpoint boundary probe timed out: main guest thread={} (handle=0x{:08X}) exists "
          "and is running, but did not reach a recompiled function boundary in time -- it is "
          "blocked outside recompiled code (host call, wait, or a single function looping "
          "without calling out). Last known (unsynchronized) function=0x{:08X} "
          "instruction=0x{:08X}. Last indirect call: target=0x{:08X} caller=0x{:08X} "
          "callsite=0x{:08X} r3=0x{:08X} r4=0x{:08X}. Last dispatch: target=0x{:08X} "
          "kind=0x{:X} argc=0x{:X} r3=0x{:08X} r4=0x{:08X}. Guest return-addr chain:{}. "
          "DEBUG PROBE object@0x8342B654 field0=0x{:08X}",
          result.thread_id, result.thread_handle, result.last_known_function,
          result.last_known_instruction, result.last_indirect_target,
          result.last_indirect_caller, result.last_indirect_callsite, result.last_indirect_r3,
          result.last_indirect_r4, result.last_dispatch_target, result.last_dispatch_kind,
          result.last_dispatch_arg_count, result.last_dispatch_r3, result.last_dispatch_r4,
          result.guest_return_chain, result.debug_probe_object_field0);
    } else {
      REXLOG_WARN(
          "Checkpoint boundary probe found no running main guest thread (is_guest_thread() && "
          "main_thread() && is_running() matched nothing)");
    }
  });
  rex::ui::RegisterBind("bind_thread_snapshot", "F6", "Snapshot all guest threads", [] {
    const auto snapshots = system::SnapshotAllGuestThreads();
    REXLOG_INFO("[THREADSNAP] {} guest thread(s):", snapshots.size());
    for (const auto& snap : snapshots) {
      REXLOG_INFO(
          "[THREADSNAP] id={} handle=0x{:08X} main={} running={} last_known_function=0x{:08X} "
          "last_known_instruction=0x{:08X}",
          snap.thread_id, snap.handle, snap.is_main, snap.is_running, snap.last_known_function,
          snap.last_known_instruction);
    }
  });
  rex::ui::RegisterBind("bind_calltrace_dump", "F8", "Dump main thread [CALLTRACE] buffer", [] {
    const auto trace = system::DumpMainThreadCallTrace();
    if (trace.empty()) {
      REXLOG_WARN(
          "[CALLTRACE-DUMP] empty (calltrace_kernel_calls is off, no main thread, or no calls "
          "recorded yet)");
      return;
    }
    REXLOG_INFO("[CALLTRACE-DUMP] {} call(s), oldest first:", trace.size());
    std::string line;
    for (size_t i = 0; i < trace.size(); ++i) {
      line += trace[i];
      line += ' ';
      if ((i + 1) % 16 == 0 || i + 1 == trace.size()) {
        REXLOG_INFO("[CALLTRACE-DUMP] {}", line);
        line.clear();
      }
    }
  });
  rex::ui::RegisterBind(
      "bind_calltrace_dump_thread8", "F9", "Dump guest thread id=7 and id=8 [CALLTRACE] buffers",
      [] {
        for (uint32_t tid : {7u, 8u}) {
          const auto trace = system::DumpThreadCallTraceById(tid);
          if (trace.empty()) {
            REXLOG_WARN(
                "[CALLTRACE-DUMP-T{}] empty (calltrace_kernel_calls is off, thread id={} not "
                "found, or no calls recorded yet)",
                tid, tid);
            continue;
          }
          REXLOG_INFO("[CALLTRACE-DUMP-T{}] {} call(s), oldest first:", tid, trace.size());
          std::string line;
          for (size_t i = 0; i < trace.size(); ++i) {
            line += trace[i];
            line += ' ';
            if ((i + 1) % 16 == 0 || i + 1 == trace.size()) {
              REXLOG_INFO("[CALLTRACE-DUMP-T{}] {}", tid, line);
              line.clear();
            }
          }
        }
