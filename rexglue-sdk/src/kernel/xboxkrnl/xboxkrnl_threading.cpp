...
uint32_t xeKeReleaseSemaphore(X_KSEMAPHORE* semaphore_ptr, uint32_t increment, uint32_t adjustment,
  ...
  // [WAITDIAG-RELEASE] 2026-09-02: correlating against a main-thread stall
  // parked forever in NtWaitForSingleObjectEx on a Semaphore handle -- see
  // if this specific handle is ever released by anyone.
  REXKRNL_WARN("[WAITDIAG-RELEASE] thread={:#x} Ke handle={:#010x} adjustment={} "
               "previous_count={} success={}",
               XThread::GetCurrentThreadHandle(), sem->handle(), adjustment, previous_count,
               success);
...
u32 NtReleaseSemaphore_entry(u32 sem_handle, u32 release_count, mapped_u32 previous_count_ptr) {
  ...
  if (sem) {
    ...
    // [WAITDIAG-RELEASE] 2026-09-02: see xeKeReleaseSemaphore for context.
    REXKRNL_WARN("[WAITDIAG-RELEASE] thread={:#x} Nt handle={:#010x} release_count={} "
                 "previous_count={} success={}",
                 XThread::GetCurrentThreadHandle(), sem_handle, release_count, previous_count,
                 success);
  } else {
    result = X_STATUS_INVALID_HANDLE;
  }
...
u32 NtWaitForSingleObjectEx_entry(u32 object_handle, u32 wait_mode, u32 alertable,
  ...
  if (object) {
    uint64_t timeout = timeout_ptr ? static_cast<uint64_t>(*timeout_ptr) : 0u;
    // Only log genuinely-blocking waits: no timeout at all (blocks until
    // signaled), or a relative timeout longer than ~200ms (200,000 in
    // 100ns units below UINT64_MAX, since relative timeouts are encoded as
    // negative -- a SMALLER unsigned value here means a LONGER real wait).
    // Tight per-frame polls (timeout=0 or a few ms, i.e. unsigned value
    // very close to UINT64_MAX) are excluded -- they were flooding the log
    // fast enough to rotate away the run's own startup lines.
    bool interesting = !timeout_ptr || timeout < (~0ULL - 2000000ull);
    if (interesting) {
      REXKRNL_WARN("[WAITDIAG] thread={:#x} handle={:#010x} type={} name='{}' timeout={:#x} "
                   "alertable={}",
                   XThread::GetCurrentThreadHandle(), object_handle,
                   static_cast<uint32_t>(object->type()), object->name(), timeout, alertable);
    }
    result = object->Wait(3, wait_mode, alertable, timeout_ptr ? &timeout : nullptr);
    if (interesting) {
      REXKRNL_WARN("[WAITDIAG] thread={:#x} handle={:#010x} RETURNED result={:#010x}",
                   XThread::GetCurrentThreadHandle(), object_handle, (uint32_t)result);
    }
    if (alertable && result == X_STATUS_USER_APC) {
      XThread::GetCurrentThread()->DeliverAPCs();
    }
...
u32 NtSignalAndWaitForSingleObjectEx_entry(u32 signal_handle, u32 wait_handle, u32 alertable,
  ...
  if (signal_object && wait_object) {
    uint64_t timeout = timeout_ptr ? static_cast<uint64_t>(*timeout_ptr) : 0u;
    // [WAITDIAG] Same filter as NtWaitForSingleObjectEx_entry above: only log
    // genuinely-blocking waits (see the comment there for the threshold
    // reasoning). This call was NOT previously instrumented, unlike the
    // plain NtWaitForSingleObjectEx -- confirmed (2026-09-02) via a
    // checkpoint-boundary probe that the main guest thread can go silent for
    // 30+ seconds parked right after a call into this exact function, with
    // no prior WAITDIAG coverage to show what it was waiting on.
    bool interesting = !timeout_ptr || timeout < (~0ULL - 2000000ull);
    if (interesting) {
      REXKRNL_WARN(
          "[WAITDIAG-SIGNAL] thread={:#x} signal_handle={:#010x} signal_type={} "
          "signal_name='{}' wait_handle={:#010x} wait_type={} wait_name='{}' timeout={:#x} "
          "alertable={}",
          XThread::GetCurrentThreadHandle(), signal_handle,
          static_cast<uint32_t>(signal_object->type()), signal_object->name(), wait_handle,
          static_cast<uint32_t>(wait_object->type()), wait_object->name(), timeout, alertable);
    }
    result = XObject::SignalAndWait(signal_object.get(), wait_object.get(), 3, 1, alertable,
                                    timeout_ptr ? &timeout : nullptr);
    if (interesting) {
      REXKRNL_WARN("[WAITDIAG-SIGNAL] thread={:#x} wait_handle={:#010x} RETURNED result={:#010x}",
                   XThread::GetCurrentThreadHandle(), wait_handle, (uint32_t)result);
    }
  } else {
    result = X_STATUS_INVALID_HANDLE;
  }
