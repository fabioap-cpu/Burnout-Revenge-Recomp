...
static_assert_size(XECRYPT_SHA512_STATE, 0xC8);

using XECRYPT_SHA384_STATE = XECRYPT_SHA512_STATE;

struct FFmpegSha512Mirror {
  uint8_t digest_len;
  uint64_t count;
  uint8_t buffer[128];
  uint64_t state[8];
...
static void XeCryptSha512FamilyInit(ppc_ptr_t<XECRYPT_SHA512_STATE> sha_state, int bits) {
  sha_state.Zero();
  std::vector<uint8_t> ctx_buf(av_sha512_size);
  av_sha512_init(reinterpret_cast<AVSHA512*>(ctx_buf.data()), bits);
  auto* mirror = reinterpret_cast<FFmpegSha512Mirror*>(ctx_buf.data());
  std::copy_n(mirror->state, rex::countof(sha_state->state), sha_state->state);
}

static void XeCryptSha512FamilyUpdate(ppc_ptr_t<XECRYPT_SHA512_STATE> sha_state, int bits,
                                      mapped_void input, u32 input_size) {
  std::vector<uint8_t> ctx_buf(av_sha512_size);
  auto* ctx = reinterpret_cast<AVSHA512*>(ctx_buf.data());
  auto* mirror = reinterpret_cast<FFmpegSha512Mirror*>(ctx_buf.data());
  mirror->digest_len = static_cast<uint8_t>(bits >> 6);
  mirror->count = sha_state->count;
  std::copy(std::begin(sha_state->state), std::end(sha_state->state), mirror->state);
  std::copy(std::begin(sha_state->buffer), std::end(sha_state->buffer), mirror->buffer);
  av_sha512_update(ctx, static_cast<const uint8_t*>(input), input_size);
  sha_state->count = mirror->count;
  std::copy_n(mirror->state, rex::countof(sha_state->state), sha_state->state);
  std::copy_n(mirror->buffer, rex::countof(sha_state->buffer), sha_state->buffer);
}

static void XeCryptSha512FamilyFinal(ppc_ptr_t<XECRYPT_SHA512_STATE> sha_state, int bits,
                                     ppc_ptr_t<uint8_t> out, u32 out_size) {
  std::vector<uint8_t> ctx_buf(av_sha512_size);
  auto* ctx = reinterpret_cast<AVSHA512*>(ctx_buf.data());
  auto* mirror = reinterpret_cast<FFmpegSha512Mirror*>(ctx_buf.data());
  mirror->digest_len = static_cast<uint8_t>(bits >> 6);
  ...
  std::copy(std::begin(sha_state->buffer), std::end(sha_state->buffer), mirror->buffer);
  uint8_t hash[64];
  av_sha512_final(ctx, hash);
  std::copy_n(hash, std::min<size_t>(bits / 8, out_size), static_cast<uint8_t*>(out));
}

void XeCryptSha384Init_entry(ppc_ptr_t<XECRYPT_SHA384_STATE> sha_state) {
  XeCryptSha512FamilyInit(sha_state, 384);
}
void XeCryptSha384Update_entry(ppc_ptr_t<XECRYPT_SHA384_STATE> sha_state, mapped_void input, u32 input_size) {
  XeCryptSha512FamilyUpdate(sha_state, 384, input, input_size);
}
void XeCryptSha384Final_entry(ppc_ptr_t<XECRYPT_SHA384_STATE> sha_state, ppc_ptr_t<uint8_t> out, u32 out_size) {
  XeCryptSha512FamilyFinal(sha_state, 384, out, out_size);
}
void XeCryptSha512Init_entry(ppc_ptr_t<XECRYPT_SHA512_STATE> sha_state) {
  XeCryptSha512FamilyInit(sha_state, 512);
}
void XeCryptSha512Update_entry(ppc_ptr_t<XECRYPT_SHA512_STATE> sha_state, mapped_void input, u32 input_size) {
  XeCryptSha512FamilyUpdate(sha_state, 512, input, input_size);
}
void XeCryptSha512Final_entry(ppc_ptr_t<XECRYPT_SHA512_STATE> sha_state, ppc_ptr_t<uint8_t> out, u32 out_size) {
  XeCryptSha512FamilyFinal(sha_state, 512, out, out_size);
}
...
// XeCryptSha384Init/Update/Final and XeCryptSha512Init/Update/Final are
// implemented above via FFmpeg's av_sha512_* API. Only the one-shot
// XeCryptSha384/XeCryptSha512 forms stay stubbed -- FH2's generated code
// never calls them.
...
REX_EXPORT(__imp__XeCryptSha384Init, rex::kernel::xboxkrnl::XeCryptSha384Init_entry)
REX_EXPORT(__imp__XeCryptSha384Update, rex::kernel::xboxkrnl::XeCryptSha384Update_entry)
REX_EXPORT(__imp__XeCryptSha384Final, rex::kernel::xboxkrnl::XeCryptSha384Final_entry)
