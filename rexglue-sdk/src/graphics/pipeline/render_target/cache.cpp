...
bool RenderTargetCache::Update(bool is_rasterization_done,
  ...
  if (is_rasterization_done) {
    ...
    for (uint32_t i = 0; i < xenos::kMaxColorRenderTargets; ++i) {
      ...
      if (REXCVAR_GET(diag_rt_format)) {
        static uint32_t last_logged_format[xenos::kMaxColorRenderTargets] = {
            UINT32_MAX, UINT32_MAX, UINT32_MAX, UINT32_MAX};
        uint32_t fmt_u32 = uint32_t(color_format);
        if (last_logged_format[i] != fmt_u32) {
          last_logged_format[i] = fmt_u32;
          uint32_t log_base = color_info.color_base;
          REXGPU_INFO("[DIAG] render_target[{}] color_format={} base={:X}", i, fmt_u32, log_base);
        }
      }
