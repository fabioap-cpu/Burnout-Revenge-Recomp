...
REXCVAR_DEFINE_BOOL(diag_blend_constant, false, "Diagnostics",
                    "Log the constant blend color (fixed-function and ROV paths) whenever it "
      ...
      [this, &swap_texture_srv_desc, frontbuffer_format, swap_texture_resource, guest_output_width,
       guest_output_height](ui::Presenter::GuestOutputRefreshContext& context) -> bool {
        ...
        if (REXCVAR_GET(diag_gamma_ramp)) {
          static bool logged_pwl_mode = false;
          static bool last_use_pwl = false;
          if (!logged_pwl_mode || last_use_pwl != use_pwl_gamma_ramp) {
            logged_pwl_mode = true;
            last_use_pwl = use_pwl_gamma_ramp;
            REXGPU_INFO("[DIAG] use_pwl_gamma_ramp={} frontbuffer_format={}", use_pwl_gamma_ramp,
...
void D3D12CommandProcessor::UpdateFixedFunctionState(
  ...
  if (render_target_cache_->GetPath() == RenderTargetCache::Path::kHostRenderTargets) {
    ...
    if (ff_blend_factor_update_needed_) {
      ...
      if (REXCVAR_GET(diag_blend_constant)) {
        static float last_logged_ff_blend[4] = {-1.0f, -1.0f, -1.0f, -1.0f};
        if (last_logged_ff_blend[0] != blend_factor[0] ||
            last_logged_ff_blend[1] != blend_factor[1] ||
            last_logged_ff_blend[2] != blend_factor[2] ||
            last_logged_ff_blend[3] != blend_factor[3]) {
          std::memcpy(last_logged_ff_blend, blend_factor, sizeof(float) * 4);
          REXGPU_INFO("[DIAG] fixed_function_blend_factor R={:.3f} G={:.3f} B={:.3f} A={:.3f}",
...
void D3D12CommandProcessor::UpdateSystemConstantValues(
  ...
  if (edram_rov_used) {
    ...
    system_constants_.edram_blend_constant[3] = regs.Get<float>(XE_GPU_REG_RB_BLEND_ALPHA);
    if (REXCVAR_GET(diag_blend_constant)) {
      static float last_logged_blend[4] = {-1.0f, -1.0f, -1.0f, -1.0f};
      float r = system_constants_.edram_blend_constant[0];
      float g = system_constants_.edram_blend_constant[1];
      float b = system_constants_.edram_blend_constant[2];
      float a = system_constants_.edram_blend_constant[3];
      if (last_logged_blend[0] != r || last_logged_blend[1] != g || last_logged_blend[2] != b ||
          last_logged_blend[3] != a) {
        last_logged_blend[0] = r;
        last_logged_blend[1] = g;
        last_logged_blend[2] = b;
        last_logged_blend[3] = a;
        REXGPU_INFO("[DIAG] rov_blend_constant R={:.3f} G={:.3f} B={:.3f} A={:.3f}", r, g, b, a);
      }
    }
  }
