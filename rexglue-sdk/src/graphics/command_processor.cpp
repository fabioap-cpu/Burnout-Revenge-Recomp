...
REXCVAR_DEFINE_BOOL(diag_draw_rate, false, "Diagnostics",
                    "Log PM4 draw packets processed per second. A sudden multiple of the "
                    "normal rate (e.g. while paused) can indicate a runaway resubmission "
                    "loop rather than a real increase in scene complexity.");
REXCVAR_DEFINE_BOOL(diag_gamma_ramp, false, "Diagnostics",
                    "Log gamma ramp values (256-entry table and PWL) as the guest writes "
                    "them, at a few sample indices. Useful to rule out gamma-ramp-driven "
                    "color issues by confirming whether the ramp is identity (R=G=B).");
    ...
    switch (index) {
      ...
      case XE_GPU_REG_DC_LUT_SEQ_COLOR: {
        ...
        if (write_gamma_ramp_component) {
          ...
          if (REXCVAR_GET(diag_gamma_ramp) &&
              (gamma_ramp_rw_index.rw_index == 0 || gamma_ramp_rw_index.rw_index == 128 ||
               gamma_ramp_rw_index.rw_index == 255)) {
            uint32_t log_r = gamma_ramp_entry.color_10_red;
            uint32_t log_g = gamma_ramp_entry.color_10_green;
            uint32_t log_b = gamma_ramp_entry.color_10_blue;
            uint32_t log_idx = gamma_ramp_rw_index.rw_index;
            REXGPU_INFO(
                "[DIAG] SEQ_COLOR idx={} comp_seq={} -> R={} G={} B={}", log_idx,
                gamma_ramp_rw_component_, log_r, log_g, log_b);
          }
        }
      ...
      case XE_GPU_REG_DC_LUT_PWL_DATA: {
        ...
        if (write_gamma_ramp_component) {
          ...
          if (REXCVAR_GET(diag_gamma_ramp) &&
              (gamma_ramp_rw_index_pwl == 0 || gamma_ramp_rw_index_pwl == 127)) {
            uint32_t log_base = gamma_ramp_entry.base;
            uint32_t log_delta = gamma_ramp_entry.delta;
            REXGPU_INFO(
                "[DIAG] idx={} component_seq={} base={} delta={}",
                gamma_ramp_rw_index_pwl, gamma_ramp_rw_component_, log_base, log_delta);
          }
        }
        if (++gamma_ramp_rw_component_ >= 3) {
          gamma_ramp_rw_component_ = 0;
      ...
      case XE_GPU_REG_DC_LUT_30_COLOR: {
        ...
        if (gamma_ramp_write_enable_mask) {
          ...
          if (REXCVAR_GET(diag_gamma_ramp) &&
            ...
            uint32_t log_idx = gamma_ramp_rw_index.rw_index;
            REXGPU_INFO(
                "[DIAG] 30_COLOR idx={} mask={} -> R={} G={} B={}", log_idx,
                gamma_ramp_write_enable_mask, log_r, log_g, log_b);
          }
        }
...
bool CommandProcessor::ExecutePacketType3Draw(memory::RingBuffer* reader, uint32_t packet,
  ...
  if (REXCVAR_GET(diag_draw_rate)) {
    static std::atomic<uint64_t> draw_packet_count{0};
    static auto last_report = std::chrono::steady_clock::now();
    draw_packet_count.fetch_add(1, std::memory_order_relaxed);
    auto now = std::chrono::steady_clock::now();
    if (now - last_report >= std::chrono::seconds(1)) {
      last_report = now;
      REXGPU_INFO("[DIAG] draw_packets_per_sec={}",
                  draw_packet_count.exchange(0, std::memory_order_relaxed));
    }
  }
