...
int XmaContext::PrepareDecoder(int sample_rate, bool is_two_channel) {
  ...
  if (av_context_->sample_rate != sample_rate ||
    ...
    // 2026-09-03: this FFmpeg reinit sequence (avcodec_free_context /
    // avcodec_alloc_context3 / avcodec_open2 -- WMA Pro / MDCT table init)
    // was measured to clobber the thread's MXCSR exception masks instead of
    // restoring them. Two guards are needed together, not either alone
    // (measured, not assumed):
    //   1. Here, immediately before the sequence -- without it, the crash
    //      happens *inside* avcodec_open2/ff_mdct_init itself, before any
    //      translated guest function ever gets a chance to run its own
    //      prologue guard.
    //   2. In every translated function's prologue
    //      (rex_ensure_safe_fp_exception_masks() in the generated pch.h) --
    //      without it, corruption from this call was shown to outlive this
    //      function and crash unrelated guest code running later on the same
    //      thread instead.
    // See Burnout Revenge PROJECT_STATE.md "ff_mdct_init STATUS_FLOAT_
    // INVALID_OPERATION" for the measurements behind both.
