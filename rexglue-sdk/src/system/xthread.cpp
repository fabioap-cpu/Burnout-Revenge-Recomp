...
CheckpointProbeResult ProbeMainThreadCheckpoint(uint32_t timeout_ms) {
  ...
  result.thread_found = true;
  result.thread_id = target->thread_id();
  result.thread_handle = target->handle();
  ...
  if (!reached) {
    // The thread never reached a checkpoint to cooperatively report its own
    // state, so peek at its PPCContext directly instead. Unsynchronized --
    // see the comment on last_known_function/last_known_instruction.
    if (auto* thread_state = target->thread_state()) {
      if (auto* context = thread_state->context()) {
        result.last_known_function = static_cast<uint32_t>(context->current_function);
        result.last_known_instruction = static_cast<uint32_t>(context->current_instruction);
        result.last_indirect_target = context->last_indirect_target;
        result.last_indirect_caller = context->last_indirect_caller;
        result.last_indirect_callsite = context->last_indirect_callsite;
        result.last_indirect_r3 = context->last_indirect_r3;
        result.last_indirect_r4 = context->last_indirect_r4;
        result.last_dispatch_target = context->last_dispatch_target;
        result.last_dispatch_kind = context->last_dispatch_kind;
        result.last_dispatch_arg_count = context->last_dispatch_arg_count;
        result.last_dispatch_r3 = context->last_dispatch_r3;
        result.last_dispatch_r4 = context->last_dispatch_r4;
        // Same code-range heuristic as [AVDIAG] in exception_handler_win.cpp.
        auto* mem_base = rex::system::kernel_state()->memory()->virtual_membase();
        uint32_t sp = context->r1.u32;
        std::string chain;
        for (int i = 0; i < 64 && sp >= 0x1000 && sp < 0xC0000000; ++i) {
          uint32_t w = __builtin_bswap32(*reinterpret_cast<uint32_t*>(mem_base + sp + i * 4));
          if ((w >= 0x82000000 && w < 0x831F0000) || (w >= 0x88050000 && w < 0x88250000))
            chain += fmt::format(" 0x{:08X}", w);
        }
        result.guest_return_chain = chain;
        result.debug_probe_object_field0 =
            __builtin_bswap32(*reinterpret_cast<uint32_t*>(mem_base + 0x8342B654u));
      }
    }
  }
...
std::vector<const char*> DumpMainThreadCallTrace() {
  ...
  auto* thread_state = target->thread_state();
  if (!thread_state) {
    return result;
  }
  ...
  const uint32_t count = std::min(context->call_trace_index, PPCContext::kCallTraceCapacity);
  const uint32_t start = context->call_trace_index - count;
  result.reserve(count);
  for (uint32_t sequence = start; sequence < context->call_trace_index; ++sequence) {
    const auto& entry = context->call_trace[sequence & (PPCContext::kCallTraceCapacity - 1)];
    result.push_back(entry.name ? entry.name : "?");
  }
  return result;
}
...
std::vector<const char*> DumpThreadCallTraceById(uint32_t guest_thread_id) {
  ...
  object_ref<XThread> target;
  for (auto thread : runtime->kernel_state()->object_table()->GetObjectsByType<XThread>()) {
    if (thread->is_guest_thread() && thread->thread_id() == guest_thread_id) {
      target = thread;
      break;
    }
  }
...
std::vector<GuestThreadSnapshot> SnapshotAllGuestThreads() {
  ...
  for (auto thread : runtime->kernel_state()->object_table()->GetObjectsByType<XThread>()) {
    ...
    GuestThreadSnapshot snap;
    snap.thread_id = thread->thread_id();
    snap.handle = thread->handle();
    snap.is_main = thread->main_thread();
    snap.is_running = thread->is_running();
    if (auto* thread_state = thread->thread_state()) {
      if (auto* context = thread_state->context()) {
        snap.last_known_function = static_cast<uint32_t>(context->current_function);
        snap.last_known_instruction = static_cast<uint32_t>(context->current_instruction);
      }
    }
