...
// [CALLTRACE] Comparison-with-Xenia diagnostic (2026-09-02, rebuilt as a
// buffer): records every kernel/CRT export call by name into a per-thread
// ring buffer on PPCContext (see call_trace/kCallTraceCapacity), matching
// Xenia's "d> handle Name(...)" debug trace conceptually, without the
// synchronous-logging version's cost. Dump with DumpCallTrace()
// (system/checkpoint.h) on demand or on crash -- never printed here. Gated
// by the calltrace_kernel_calls cvar (default off) so it costs nothing
...
#define REX_HOOK(subroutine, function)                                                  \
  extern "C" REX_FUNC(subroutine) {                                                     \
    ...
    if (REXCVAR_GET(calltrace_kernel_calls)) {                                          \
      ctx.call_trace[ctx.call_trace_index & (PPCContext::kCallTraceCapacity - 1)].name = \
          #subroutine;                                                                  \
      ++ctx.call_trace_index;                                                           \
    }                                                                                   \
